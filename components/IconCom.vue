<template>
	<!-- 全局固定客服联系按钮 -->
	<button v-if="showCustomerService" class="customer-service-btn" :class="{ dragging: isDragging }"
		:style="{ left: currentX + 'px', top: currentY + 'px' }" open-type="contact" @touchstart="onTouchStart"
		@touchmove="onTouchMove" @touchend="onTouchEnd" @click="handleCustomerClick" @contact="onContact"
		@getphonenumber="onGetPhoneNumber" @error="onError">
	...
	export default {
		...
		data() {
			return {
				...
				showCustomerService: true // 控制是否显示客服按钮
			}
		},
